import { masonryLayout, COLUMN_WIDTH, SUB_TILE_SIZE, GAP } from './masonryLayout';

describe('masonryLayout', () => {
  it('gives a single entry with one photo one big square tile', () => {
    const result = masonryLayout([['a']]);

    expect(result.rects).toEqual([{ x: 0, y: 0, width: COLUMN_WIDTH, height: COLUMN_WIDTH }]);
  ...
  it('places each single-photo entry as a big tile in the shorter column', () => {
    ...
    expect(result.rects[0]).toEqual({ x: 0, y: 0, width: COLUMN_WIDTH, height: COLUMN_WIDTH });
  ...
  it('shrinks an entry\'s extra photos into a sub-grid beneath its big tile, uncapped', () => {
    ...
    expect(result.rects[1]).toEqual({ x: 0, y: rowY, width: SUB_TILE_SIZE, height: SUB_TILE_SIZE });
    ...
    expect(result.rects[3]).toEqual({ x: 0, y: secondRowY, width: SUB_TILE_SIZE, height: SUB_TILE_SIZE });
  ...
  it('produces no rects for an empty group', () => {
    expect(masonryLayout([])).toEqual({ rects: [], unitWidth: 1000, unitHeight: 0 });
  });
});
